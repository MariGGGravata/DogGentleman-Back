...
-- --------------------------------------------------
-- Date Created: 11/19/2020 18:16:27
...
-- Creating table 'RaçãoSet'
CREATE TABLE [dbo].[RaçãoSet] (
...
-- Creating primary key on [Id] in table 'RaçãoSet'
ALTER TABLE [dbo].[RaçãoSet]
ADD CONSTRAINT [PK_RaçãoSet]
...
-- Creating foreign key on [Cadastro_Id] in table 'ProdutoSet'
ALTER TABLE [dbo].[ProdutoSet]
ADD CONSTRAINT [FK_CadastroProduto]
    FOREIGN KEY ([Cadastro_Id])
    REFERENCES [dbo].[RaçãoSet]
        ([Id])
